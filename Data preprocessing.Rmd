...
During data exploration, a univariate Anova showed significant differences between the mean diversity across the study groups. 

```{r}
anova_diversity_group <- aov(diversity ~ Study.Group, data = merged_df)
...
```{r}
mlm_Study.Group <- multinom(Study.Group ~ diversity + fecalcal + consent_age + Weight_at_baseline, data = merged_df)
...
```{r}
levels(merged_df$Study.Group)
merged_df$Study.Group <-relevel(merged_df$Study.Group,ref = "nonIBD")
levels(merged_df$Study.Group)
```
...
```{r}
coef_matrix <- summary_mlm$coefficients
coef_matrix
se_matrix <- summary_mlm$standard.errors
se_matrix
...
```{r}
merged_df_subset_CD_nonIBD <- merged_df_subset_CD_nonIBD %>%
  mutate(Study.Group = ifelse(Study.Group %in% c("CD"), 1, 0))

merged_df_subset_UC_nonIBD <- merged_df_subset_UC_nonIBD %>%
...
```{r}
blm_fecalcal_1 <- glm(Study.Group ~ fecalcal, data=merged_df_subset_CD_nonIBD, family="binomial" )
...
```{r}
plot_blm_1 <- ggplot(merged_df_subset_CD_nonIBD, aes(x = fecalcal, y = Study.Group)) +
  geom_point() +
...
```{r}
blm_fecalcal_2 <- glm(Study.Group ~ fecalcal, data=merged_df_subset_UC_nonIBD, family="binomial" )
...
```{r}
plot_blm_2<- ggplot(merged_df_subset_UC_nonIBD, aes(x = fecalcal, y = Study.Group)) +
  geom_point() +
